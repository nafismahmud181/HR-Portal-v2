...
export default function DocumentCommSection({
  formData,
  onNestedInputChange,
}: DocumentCommSectionProps) {
  ...
  const addSignatory = async () => {
    if (
      newSignatory.name.trim() &&
      newSignatory.title.trim() &&
      newSignatory.email.trim()
    ) {
      setSavingSignatory(true);
      try {
        const updatedSignatories = [
          ...formData.documentConfig.authorizedSignatories,
          { ...newSignatory },
        ];
        onNestedInputChange(
          "documentConfig",
        ...
        console.log(
          "Signatory added successfully to local state:",
          newSignatory
        );
        console.log(
          "Total signatories in local state:",
          updatedSignatories.length
        );
        console.log(
          "⚠️ IMPORTANT: Click 'Save Changes' at bottom of page to persist to Firebase!"
        );
      } catch (error) {
  ...
  const updateSignatory = async (index: number) => {
    ...
    try {
      const updatedSignatories =
        formData.documentConfig.authorizedSignatories.map((signatory, i) =>
      ...
      console.log(
        "Signatory updated successfully. Remember to save changes to persist to Firebase."
      );
    } catch (error) {
  ...
  const removeSignatory = async (index: number) => {
    ...
    try {
      const updatedSignatories =
        formData.documentConfig.authorizedSignatories.filter(
          (_, i) => i !== index
        );
      onNestedInputChange(
        "documentConfig",
      ...
      console.log(
        "Signatory removed successfully. Remember to save changes to persist to Firebase."
      );
    } catch (error) {
      ...
      <section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
              ...
              <div className="space-y-2">
                ...
                {/* Validation Errors */}
                {!employeeIdValidation.valid &&
                  employeeIdValidation.errors.length > 0 && (
                ...
                {employeeIdValidation.valid && employeeIdPreview && (
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        setShowEmployeeIdPreview(!showEmployeeIdPreview)
                      }
                ...
                {showEmployeeIdPreview && employeeIdValidation.valid && (
                  <div className="mt-2 p-3 bg-[#f9fafb] border border-[#e5e7eb] rounded-lg">
                    <div className="text-[12px] font-medium text-[#374151] mb-2">
                      Example IDs:
                    </div>
                    <div className="space-y-1">
                      {generateEmployeeIdPreview(
                        ...
                        <div key={index} className="flex items-center gap-2">
                          <span className="text-[11px] text-[#6b7280] w-8">
                            #{index + 1}:
                          </span>
          ...
          <div className="mt-4 p-4 bg-[#f9fafb] border border-[#e5e7eb] rounded-lg">
            ...
            <div className="mt-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">
              ...
              <ul className="text-[11px] text-blue-700 space-y-1">
                <li>
                  • Use <code>{"{###}"}</code> for 3-digit sequential numbers
                  (001, 002, 003...)
                </li>
                <li>
                  • Use <code>{"{####}"}</code> for 4-digit sequential numbers
                  (0001, 0002, 0003...)
                </li>
                <li>
                  • Sequential numbers are automatically generated and unique
                </li>
                <li>
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-[12px] text-[#6b7280] mt-1">
                Signatory changes are saved locally. Click &quot;Save
                Changes&quot; at the bottom of the page to persist to Firebase.
              </p>
            </div>
            <button
            ...
            <div className="bg-[#f9fafb] border border-[#e5e7eb] rounded-lg p-6 mb-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="md:col-span-2">
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      setNewSignatory((prev) => ({
                        ...prev,
                        signatureTemplate: e.target.value,
                      }))
                    }
                ...
                <div>
                  ...
                  />
                  <p className="text-[12px] text-[#6b7280] mt-1">
                    This email will be treated as the company email for this
                    signatory
                  </p>
                </div>
              </div>
            ...
            {formData.documentConfig.authorizedSignatories.length === 0 ? (
              <div className="text-center py-8 text-[#6b7280]">
                <p className="text-[14px]">
                  No authorized signatories added yet.
                </p>
                <p className="text-[12px] mt-1">
                  Click &quot;Add Signatory&quot; to get started.
                </p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full border border-[#e5e7eb] rounded-lg">
                  ...
                  <tbody>
                    {formData.documentConfig.authorizedSignatories.map(
                      (signatory, index) => (
            ...
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
              <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
                ...
                <p className="text-[14px] text-[#6b7280] mb-6">
                  ...
                  {
                    formData.documentConfig.authorizedSignatories[confirmDelete]
                      ?.name
                  }
                  &quot; from the authorized signatories?
                </p>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
              <label className="block text-[14px] font-medium mb-2">
                Employee Records (Years)
              </label>
              <input
                type="number"
                value={formData.documentConfig.retentionPolicy.employeeRecords}
                onChange={(e) =>
                  onNestedInputChange("documentConfig", "retentionPolicy", {
            ...
            <div>
              <label className="block text-[14px] font-medium mb-2">
                Performance Reviews (Years)
              </label>
              <input
                type="number"
                value={
                  formData.documentConfig.retentionPolicy.performanceReviews
                }
                onChange={(e) =>
                  onNestedInputChange("documentConfig", "retentionPolicy", {
                    ...formData.documentConfig.retentionPolicy,
          ...
          <div className="space-y-6">
            <div>
              ...
              <textarea
                ...
                onChange={(e) =>
                  onNestedInputChange("communicationPrefs", "emailTemplates", {
              ...
              />
              <p className="mt-1 text-[12px] text-[#6b7280]">
                Available variables: {"{companyName}"}, {"{employeeName}"},{" "}
                {"{startDate}"}
              </p>
            </div>
            ...
            <div>
              ...
              <textarea
                value={
                  formData.communicationPrefs.emailTemplates.generalNotification
                }
                onChange={(e) =>
                  onNestedInputChange("communicationPrefs", "emailTemplates", {
                    ...formData.communicationPrefs.emailTemplates,
            ...
            <div className="flex items-center gap-4">
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  type="checkbox"
                  checked={
                    formData.communicationPrefs.notificationSettings.smsEnabled
                  }
                  onChange={(e) =>
                    onNestedInputChange(
                      "communicationPrefs",
            ...
            <div className="flex items-center gap-4">
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  type="checkbox"
                  checked={
                    formData.communicationPrefs.notificationSettings.pushEnabled
                  }
                  onChange={(e) =>
                    onNestedInputChange(
                      "communicationPrefs",
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  onNestedInputChange("communicationPrefs", "localization", {
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                checked={
                  formData.communicationPrefs.localization.multiLanguageSupport
                }
                onChange={(e) =>
                  onNestedInputChange("communicationPrefs", "localization", {
                    ...formData.communicationPrefs.localization,
